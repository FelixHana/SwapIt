...
public class AuthorizationConfig {
    ...
    public SecurityFilterChain authorizationServerSecurityFilterChain(HttpSecurity http) throws Exception {
        ...
        DefaultSecurityFilterChain securityFilterChain = http
                ...
                //.requestMatcher(http.getConfigurer(OAuth2AuthorizationServerConfigurer.class).getEndpointsMatcher())                // 认证失败处理
    ...
    public SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
        //TODO
        String[] whitelist = {"/doc.html", "/v3/api-docs/**", "/assets/**", "/webjars/**", "/oauth2/**", "/login", "/logout", "/register", "/user/**", "/address/**"};
        http
                .authorizeRequests()
                // 放行静态资源
    ...
    public RegisteredClientRepository registeredClientRepository(JdbcTemplate jdbcTemplate, PasswordEncoder passwordEncoder) {
        ...
        RegisteredClient newClient = RegisteredClient.withId("client")
                ...
                .tokenSettings(TokenSettings.builder()
                        .accessTokenTimeToLive(SecurityConstants.ACCESS_TOKEN_EXPIRE_TIME)
                        .refreshTokenTimeToLive(SecurityConstants.REFRESH_TOKEN_EXPIRE_TIME)
                        .build())
                .build();
    ...
    public OAuth2TokenCustomizer<JwtEncodingContext> oAuth2TokenCustomizer() {
        ...
        return context -> {
            ...
            if (context.getPrincipal().getPrincipal() instanceof SysBaseUser) {
                ...
                claims.claim("userId", user.getId().toString());
